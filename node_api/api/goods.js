		...
		db.query("select * from sys", (err, result) => {
			...
			db.query("insert into goods set ?", req.body, (err, result) => {
				if (err) {
					...
					db.query('select * from goods order by product_id DESC limit 1', (err, result) => {
						res.json({
							code: 200,
							msg: '发布成功',
							data: result[0]
...
router.get("/detail", (req, res) => {
	db.query('select * from goods where product_id = ?', req.query.product_id, (err, result) => {
		if (err) {
			throw new Error(err)
		} else {
			if (!result[0]) {
...
router.get("/list",(req,res) => {
	...
	db.query("select goods.*,user.nickname as store_name from goods inner join user where goods.store_id = user.user_id and product_name like ? and goods.status"+ status +" limit ?,?",params,(err,result) => {
		if(err){
			...
			db.query("select count(*) as records from goods where product_name like ? and status"+ status, params ,(error, results)=> {
				if (error) {
		...
		db.query("select * from sys", (err, result) => {
			...
			db.query('update goods set ? where product_id = ?', [req.body, req.body.product_id], (err, result) => {
				if (err) {
					...
					db.query('select * from goods where product_id = ?', req.body.product_id, (err, results) => {
						if (err) {
							throw new Error(err)
						} else {
							if(results[0]){
