<template>
	<view class="uni-load-more" @click="onClick">
		...
		<!-- #endif -->
		<view class="uni-load-more__text" :style="{color: color}" v-if="status!='no'">
			<text v-if="status === 'noMore'"></text>
			<view>{{ status === 'more' ? contentdownText : status === 'loading' ? contentrefreshText : contentnomoreText }}</view>
			<text v-if="status === 'noMore'"></text>
		</view>
	</view>
</template>
